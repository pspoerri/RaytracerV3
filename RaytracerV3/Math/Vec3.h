...
public:
    ...
    */
    ...
    template <typename S>
    const Vec3 & operator=(const Vec4<S>& a) {x = a.x; y = a.y; z = a.z; return *this;}
...
//template <Vec3<float> S, double T>
template<>
inline Vec3<double>
operator+(Vec3<float> s, const Vec3<double>& v)
{
    return Vec3<double>(s.x + v.x, s.y + v.y, s.z + v.z);
}
...
template <>
inline Vec3<float>
operator-(Vec3<double> s, const Vec3<float>& v)
{
    //    return Vec3<T>(0);
    return Vec3<float>(s.x - v.x, s.y - v.y, s.z - v.z);
}
...
template<typename T>
inline Vec3<T>
reflect(const Vec3<T> &n, const Vec3<T> &w)
{
    const double c = -n.dot(w);
    return w+2.0*c*n;
}
...
template<typename T>
inline Vec3<T>
refract(const Vec3<T> view, const Vec3<T> normal, double nu1nu2)
{
    Vec3<T> w = view;
    double wn = w.dot(normal);
    return -nu1nu2*(w-wn*normal)-normal*sqrt(1.0-nu1nu2*nu1nu2*(1.0-wn*wn));
}
    
// adapted from http://graphics.stanford.edu/courses/cs148-10-summer/docs/2006--degreve--reflection_refraction.pdf
// Reflectance
template<typename T>
inline double
reflectance(const Vec3<T>& normal, const Vec3<T> incident, double n1, double n2)
{
    const double n = n1/n2;
    const double cosI = -dot(normal, incident);
    const double sinT2 = n*n*(1.0-cosI*cosI);
    if (sinT2 > 1.0) return 1.0;  // Total reflection
